    ...
    try:
        ...
        if raw_detections:
            for idx, raw_det in enumerate(raw_detections):
                ...
                reasoning = f"Identified based on visual and textural anomalies occupying {area_pct}% of the road surface."
                uncertainty = f"The model suggests a {conf_pct}% probability of this classification. Please verify manually, as AI assessments are not absolutely certain."
                
                explanation = (
                    f"Detected {area_desc} {class_name.lower()} (#{idx + 1}). "
                    f"{reasoning} {uncertainty} {urgency.get(severity, '')}"
                )
